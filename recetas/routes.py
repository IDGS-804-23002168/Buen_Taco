...
def calcular_unidades_producibles(producto_id):
    ...
    minimo = None
    for r in recetas:
        # Stock actual = suma de movimientos (positivos entrada, negativos salida)
...
def agregar(producto_id):
    ...
    if request.method == 'POST':
        ...
        cantidad_base = request.form.get('cantidad_base', type=float)

        if not materia_id or not cantidad_base:
            flash('Todos los campos son obligatorios.', 'warning')
            return render_template('recetas/form.html', producto=producto, materias=materias)
    ...
    if request.method == 'POST':
        ...
        if not cantidad_base:
            ...
            return render_template('recetas/form.html',
                                   ...
                                   receta=receta)
        try:
            receta.CantidadBase = cantidad_base
            db.session.commit()
